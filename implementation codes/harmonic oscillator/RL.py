...
def set_parameters(**kwargs):
    ...
    if 'reward_multiply' in kwargs:
        global reward_multiply
...
class DQN_measurement(nn.Module):
    ...
    def __init__(self, inputs):
        ...
        k3 = 9; s3 = 4
        filters = (32, 64, 64)
        self.conv1=layers.Conv1d_weight_normalize(2, filters[0], kernel_size = k1, stride = s1, padding = 0, bias = not self.Batch_Normalize) # 1200 -> 398
        if self.Batch_Normalize: self.bn1=layers.BatchRenorm1d(filters[0])
        self.conv2=layers.Conv1d_weight_normalize(filters[0], filters[1], kernel_size = k2, stride = s2, padding = 0, bias = not self.Batch_Normalize) # 398 -> 192
        if self.Batch_Normalize: self.bn2=layers.BatchRenorm1d(filters[1])
        self.conv3=layers.Conv1d_weight_normalize(filters[1], filters[2], kernel_size = k3, stride = s3, padding = 0, bias = not self.Batch_Normalize) # 192 -> 94
        ...
        self.fc22=layers.FactorizedNoisy(256, self.num_of_control_resolution_oneside*2 + 1) # left and right means doubling, 1 extra for no control
        self.fc31=layers.Linear_weight_normalize(256, 128)
        self.fc32=layers.Linear_weight_normalize(128, 1) # and 1 extra for mean prediction
    ...
    def forward(self, x):
        ...
        x = x.view(x.size(0), -1)
        x = F.relu(self.fc1(x))
        action = F.relu(self.fc21(x))
        action = self.fc22(action)
        mean = F.relu(self.fc31(x))
...
class direct_DQN(nn.Module):
    ...
    def forward(self, x):
        x = F.relu(self.fc1(x))
        x = F.relu(self.fc2(x))
        action = F.relu(self.fc31(x))
        action = self.fc41(action)
...
class TrainDQN(object):
    ...
    def __call__(self):
        ...
        # start evaluation
        with torch.no_grad():
            next_actions = net(next_states)[0].max(1)[1]
        action_values, avg_value = net(previous_states)
        ...
        next_action_values, next_avg_value = self.target_net(next_states)
        ...
        #expected_state_action_values[rewards==failing_reward]=failing_reward
        unweighted_loss = F.mse_loss(state_action_values, expected_state_action_values, reduction='none')
        loss = (unweighted_loss*torch.from_numpy(ISWeights).to(unweighted_loss)).mean()
        loss.backward()
        self.optim.step()
        ...
        if self.report_i >= self.report_period:
            assert not np.isnan(self.accu_loss), '"NAN" encountered in loss values. Numerical error has appeared.'
            print('loss {:.2g}\t'.format(self.accu_loss/self.report_i), flush=True)
            self.clear_report()            
        return loss
...
def Load(LoadPipe, shared_data, Transitions_Sampling_Memory, Memory_Shape, batch_update_queue, inputs, tree_data, sampling_conns):
    ...
    pause_event, end_event = shared_things
    ...
    while not end_event.is_set():
        ...
        while not MemoryQueue.empty():
            ...
            if last_achieved_time.value != t_max:
                ...
                if failure_counter == 10: last_achieved_time.value = t
            ...
            # We are not sure whether a pause functionality is really needed here. It is here for consistency with other workers, but we have never
            # observed that it is triggered. So it has not been tested and there may be undiscovered bugs.
