...
parser.add_argument('--phonon_cutoff', default = 20., type=float,
                    help=r'the maximum average phonon number we allow during simulation, beyond which we will stop the simulation so as to avoid high energy samples to be learned by the AI. This is because when the learning is difficult and when the AI learns high energy samples and low energy samples simutaneously, it attempts to fit into the high energy samples more (associated with high loss) and performs worse on low energy situations, and then the accumulated experience will always contain high energy samples and the AI will achieve a bad performance in the end. To avoid this, we impose an upper bound on its learned energy region. This strategy stabilizes the learning and makes it perform better in difficult cases.')
...
args.lr_schedule = [(round(t[0]*args.train_episodes_multiplicative) if t[0]!=float('inf') else t[0], t[1]) for t in [(1000,4e-5), (3000,8e-6), (5000,2e-6), (6500,4e-7), (8000,1e-7), (9000, 0.)]]
