...
if __name__ == '__main__':
    import RL
    RL.set_parameters(control_interval=control_interval, reward_multiply=reward_multiply, t_max=t_max, F_max=args.F_max)
...
def Control(net, pipes, shared_buffer, reinforcement_learning_gamma, seed, idx):
    ...
    def do_episode():
                    ...
                    if not to_stop and phonon <= args.phonon_cutoff:
                        ...
                        if args.train:
                            experience.append(np.hstack(( last_data, data, 
                                ...
                                np.array([-(phonon-args.phonon_offset)*reward_multiply],dtype=np.float32) )) )
                    else: break
                    if t>30:
        ...
        # the organization of measurement data needs to be different:
            ...
            while not t >= t_max:
                ...
                if len(measurements_cache)==coarse_grain: 
                    ...
                    forces_along_measurements_input.append(force)
                i += 1
        ...
        t = min(t,t_max)
        avg_phonon = accu_energy/accu_counter if t >50 else args.phonon_cutoff
...
def worker_manager(net, pipes, num_of_processes, reinforcement_learning_gamma, seed, others):
    # initialize
    ...
    for n in range(num_of_processes):
        ...
        processes.append( fork.Process( target=Control, args=(copy.deepcopy(net).cpu(), (MemoryQueue, results_queue, conn2, EndEvent, PauseEvent), shared_buffer, reinforcement_learning_gamma, seed, n) ) )
    net=net.cuda()
    net.eval()
    # prepare to save
    good_actors=[(1.,0.) for i in range(num_of_saves)]
                ...
                if episode_passed > 50 and len(performances) >= 9: 
                    ...
                    if new_avg_phonon < good_actors[-1][0]:
                        ...
                        for idx, actor in enumerate(good_actors):
                            torch.save(actor[1], os.path.join(args.folder_name,'{}.pth'.format(idx+1)))
    ...
    while process_ended!=num_of_processes:
        ...
        if num_of_data == 0:
            ...
            action_values, avg_value = net(network_input[:num_of_data,:])
...
if __name__ == '__main__':
    class Main_System(object):
        ...
        def __init__(self, train, num_of_processes, others=''):
            ...
            spawn_objs = (self.MemoryInputQueue, self.end_event, self.pause_event)
            # actors
            ...
            self.worker_manager = spawn.Process( target=worker_manager, args=(copy.deepcopy(train.net).cpu(), (self.MemoryInputQueue, self.ActorReceivePipe, self.end_event, self.pause_event), num_of_processes, self.train.gamma, seed, others) )
            ...
            self.train.memory.set_memory_source(self.MemoryInputQueue, (self.pause_event, self.end_event))
        ...
        def __call__(self, num_of_episodes):
            ...
            while self.episode.value < num_of_episodes:
                ...
                if self.last_achieved_time.value == t_max: self.adjust_learning_rate()
            self.end_event.set()
        ...
        def adjust_learning_rate(self):
            change = False
            if self.episode.value >= 1000 and self.lr_step == 1:
                args.lr = min(4e-5, args.lr); change = True
            elif self.episode.value >= 3000 and self.lr_step == 2:
                args.lr = min(8e-6, args.lr); change = True
            elif self.episode.value >= 5000 and self.lr_step == 3:
                args.lr = min(2e-6, args.lr); change = True
            elif self.episode.value >= 6500 and self.lr_step == 4:
                args.lr = min(4e-7, args.lr); change = True
            elif self.episode.value >= 8000 and self.lr_step == 5:
                args.lr = min(1e-7, args.lr); change = True
            if change and args.train:
                self.lr_step += 1 
                for param_group in self.train.optim.param_groups: param_group['lr'] = args.lr
                print(colored('change learning rate to {:.2g}'.format(args.lr),attrs=['bold']))
...
if __name__ == '__main__':
    ...
    # if we test existing models, we use a loop to go over the models
    else:
        ...
        for name in glob.glob(os.path.join(args.folder_name,'*')):
            ...
            if ext=='.pth' or ext=='': test_nets.append((file_name, torch.load(name).state_dict()))
