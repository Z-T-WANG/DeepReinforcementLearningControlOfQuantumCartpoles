...
public:
    ...
    ~Set_World(){
        mkl_free(empty_rowindex_start); mkl_free(empty_rowindex_end); mkl_free(empty_column); mkl_free(empty_value);
...
static void check_boundary_error(MKL_Complex16* psi, int* Fail){
    if ( cblas_dznrm2(1, &(psi[n_max+1-5]), 1)>1.e-4 || cblas_dznrm2(1, &(psi[n_max+1-1]), 1)>1.e-4){
        *Fail = 1; }
}
/*
...
static void __attribute__((hot)) sum_them_all(MKL_Complex16* state, double dt, double dW, double dZ, MKL_Complex16* D2_state, MKL_Complex16* D1_Y_plusIm_substract_D1_Y_minusIm, MKL_Complex16* D1_Y_plusRe, \
    ...
    mkl_sparse_z_mv (SPARSE_OPERATION_NON_TRANSPOSE, {dt*dt*dt/12., 0.}, Hamiltonian_squared, {SPARSE_MATRIX_TYPE_SYMMETRIC, SPARSE_FILL_MODE_UPPER, SPARSE_DIAG_NON_UNIT}, D1_state, {0.,0.}, term7);
...
static PyObject* check_n_max(PyObject* self, PyObject *args){
    return Py_BuildValue("i", n_max);
}
...
static PyMethodDef methods[] = {
    ...
    {"check_n_max", (PyCFunction)check_n_max, METH_VARARGS,
     "test whether the imported C module responds and return n_max."},
