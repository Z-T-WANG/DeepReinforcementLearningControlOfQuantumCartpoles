...
def calculate_outside_probability(state,xth):
    state_center = state[(x_n//2-round(xth/grid_size)):(x_n//2+round(xth/grid_size))]
    prob=probability(state_center)
    return 1. - np.sum(prob)*grid_size
...
if __name__ == '__main__':
    import RL
    RL.set_parameters(control_interval=control_interval, F_max=args.F_max, failing_reward=failing_reward)
...
def Control(net, pipes, shared_buffer, seed, idx):
    ...
    xth=(args.F_max/abs(lambda_)/4*pi)**(1/3)
    ...
    def init_state():
        return Gaussian_packet(wavelength=float('inf'), mean=0., std=1.)
    # do one episode
    def do_episode():
        t = 0.
        to_stop = False
        ...
        while True:
            if not to_stop:
                to_stop=(calculate_outside_probability(state, xth)>0.5) 
            if i % control_interval == 0 and i != 0: # we do not control at the zero-th step
            ...
            i += 1
            if Fail and not to_stop: to_stop = True; numerical_failure = True
            t += time_step
...
def worker_manager(net, pipes, num_of_processes, seed, others):
    ...
    if args.test or args.control_strategy!='DQN': performances = []
...
if __name__ == '__main__':
    ...
    capacity = round(args.size_of_replay_memory*controls_per_unit_time*100.) if args.train else 1
